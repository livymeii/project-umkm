...
/* TAG LABEL */
.product-text h5 { position:absolute; top:15px; left:15px; background:red; color:white; padding:3px 10px; font-size:12px; border-radius:5px; margin:0; }
...
<section class="trending-product" id="trending">
  <div class="center-text">
    <h2>Dress <span>Collection</span></h2>
  </div>

  <div class="products">
    <?php
      $query = "SELECT p.*, k.nama_kategori FROM tb_produk p JOIN tb_kategori k ON p.id_kategori=k.id_kategori WHERE k.nama_kategori='Dress'";
...
$(document).on('click','.wishlist-toggle',function(){ const $btn=$(this); const id=$btn.data('id'); $.post('../service/add-to-wishlist.php',{id:id,action:'toggle'},function(res){ try{ var data=(typeof res==='string')?JSON.parse(res):res; }catch(e){ return; } if(data.success){ $btn.find('i').toggleClass('far fas'); updateBadge($('#wishlist-count'),data.count); } }); });
$(document).on('click','.add-to-cart',function(){ const id=$(this).data('id'); $.post('../service/add-to-cart.php',{id:id},function(res){ try{ var data=(typeof res==='string')?JSON.parse(res):res; }catch(e){ return; } if(data.success){ window.location.href='../cart.php'; } }); });
$(document).on('click','.buy-now',function(){ const id=$(this).data('id'); $.post('../service/add-to-cart.php',{id:id},function(res){ window.location.href='../checkout.php'; }); });
</script>
</body>
</html>
